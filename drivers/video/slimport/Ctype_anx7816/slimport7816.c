		...
		const char *buf, size_t count)
{
	...
	irq_enable = val;
	pr_info("%s: anx7816 irq_enable = %d\n", __func__, irq_enable);
	return count;
}
#ifdef CONFIG_SLIMPORT_COMMON
static ssize_t sp_audio(struct device *dev, struct device_attribute *attr,
		 char *buf)
{
	ex_audio();
	return 0;
...
#ifdef CONFIG_SLIMPORT_COMMON
	__ATTR(at, S_IRUSR , sp_audio, NULL),
#endif
#ifdef SP_REGISTER_SET_TEST /*slimport test */
...
int sp_read_reg(uint8_t slave_addr, uint8_t offset, uint8_t *buf)
{
	...
	if (!pdata->check_slimport_connection) {
		pr_debug("%s %s <%p>: state =  %d  read I2C communication failed because of power down",
			       LOG_TAG, __func__,__builtin_return_address(0), sp_tx_system_state);
		return -EINVAL;
	}
...
static int anx7816_system_init(void)
{
	...
	*/
	/*This function is need to initialized the variables of states machines*/
...
static irqreturn_t anx7816_cbl_det_isr(int irq, void *data)
{
	...
	static int pre_cable_state = 0;

	cable_connected = slimport_is_connected();
	pr_err("%s %s : detect cable insertion, cable_connected = %d\n",
			LOG_TAG, __func__, cable_connected);

	if (cable_connected) {
		if (pre_cable_state != cable_connected) {
			pr_err("%s %s : detect HPD High\n", LOG_TAG, __func__);
			wake_lock(&anx7816->slimport_lock);
			queue_delayed_work(anx7816->workqueue, &anx7816->work, 0);
		}
	} else {
		pr_err("%s %s : detect HPD LOW\n", LOG_TAG, __func__);
		cancel_delayed_work_sync(&anx7816->work);
		system_power_ctrl(0);
		wake_unlock(&anx7816->slimport_lock);
		wake_lock_timeout(&anx7816->slimport_lock, 2*HZ);
	}
	pre_cable_state = cable_connected;
	return IRQ_HANDLED;
}
...
static void anx7816_work_func(struct work_struct *work)
{
	...
	if (!irq_enable) {
		workqueu_timer = 2000;
		pr_info("%s: anx7816 irq_enable doesn't set yet\n", __func__);
