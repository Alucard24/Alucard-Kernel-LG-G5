...
BUILD_NOW()
{
	...
	if [ -e "$KERNELDIR"/arch/arm64/boot/Image ]; then
		...
		pagesize=4096
		cmd_line="console=ttyHSL0,115200,n8 androidboot.console=ttyHSL0 user_debug=31 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 cma=32M@0-0xffffffff androidboot.hardware=h1 dhash_entries=131072 ihash_entries=131072 androidboot.selinux=permissive enforcing=0 selinux=1"
		...
		zip -r Alucard-Kernel-"${GETVER}"-N-"$(date +"[%H-%M]-[%d-%m]-LG-MKN-PWR-CORE")".zip * >/dev/null
		stat boot.img
		rm -f ./*.img
		cd ..
	else
